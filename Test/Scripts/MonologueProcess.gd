...
func parse_events() -> void:
	for event in events:
		var variable = get_variable(event.get("Variable"))
		if not variable:
			continue
		var v_value = variable.get("Value")
		var c_value = event.get("Value")
		var condition_pass: bool
		match event.get("Operator"):
...
func process_node(node: Dictionary) -> void:
	match node.get("$type"):
		...
		"NodeAudio":
			...
			var volume = node.get("Volume")
			var pitch = node.get("Pitch")
...
func process_setter(raw_setter: Dictionary) -> void:
	match raw_setter.get("SetType"):
		...
		"ActionCustom":
			# TODO: Move out to NodeAction
			monologue_custom_action.emit(raw_setter)
...
func select_option(raw_option: Dictionary) -> void:
	...
	if raw_option.keys().has("OneShot") and raw_option.keys().has("EnableByDefault"):
